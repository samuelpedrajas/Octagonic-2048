...
func _ready():
	connect("user_input", get_node("board"), "move")
		...
		for i in range(-1, 2):
			for j in range(-1, 2):
				...
				if (v - d).length() < MINIMUM_DISTANCE_TO_MOVE:
					# change the sign of y because in the matrix is the other way round
					v.y *= -1
					emit_signal("user_input", v)

func _input_event(event):
	if event.is_action_pressed("click"):
		# if clicked, set pressed true and save the position
		tap_start_position = event.pos
		pressed = true
	elif event.is_action_released("click"):
		# if released, set pressed false and erase de position
		tap_start_position = null
		pressed = false

	# if moving while pressing, check if is necessary to make a move
